...
class ConfusionMatrix:
    ...
    def plot(self, normalize=True, save_dir='', names=(), on_plot=None):
        ...
        ax.set_xlabel('True',fontsize=19,fontweight="bold")
        ax.set_ylabel('Predicted',fontsize=19,fontweight="bold")
        ax.set_title('Confusion Matrix for YOLOv10',fontsize=20,fontweight="bold")
        plot_fname = Path(save_dir) / f'{title.lower().replace(" ", "_")}.png'
        fig.savefig(plot_fname, dpi=800)
        plt.close(fig)
        if on_plot:
            on_plot(plot_fname)
...
def shape_iou(box1, box2, xywh=False, scale=0, eps=1e-7):
    ...
    return iou  # IoU
def inner_siou(box1, box2, ratio=1.13, xywh=True, eps=1e-7):
    if xywh:  # transform from xywh to xyxy
        (x1, y1, w1, h1), (x2, y2, w2, h2) = box1.chunk(4, -1), box2.chunk(4, -1)
        w1_, h1_, w2_, h2_ = w1 / 2, h1 / 2, w2 / 2, h2 / 2
        b1_x1, b1_x2, b1_y1, b1_y2 = x1 - w1_, x1 + w1_, y1 - h1_, y1 + h1_
...
def inner_ciou(box1, box2, ratio=1.0, xywh=False, eps=1e-7):
    if xywh:  # transform from xywh to xyxy
        (x1, y1, w1, h1), (x2, y2, w2, h2) = box1.chunk(4, -1), box2.chunk(4, -1)
        w1_, h1_, w2_, h2_ = w1 / 2, h1 / 2, w2 / 2, h2 / 2
        b1_x1, b1_x2, b1_y1, b1_y2 = x1 - w1_, x1 + w1_, y1 - h1_, y1 + h1_
